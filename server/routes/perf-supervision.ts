...
router.get('/overview', authMiddleware, (req: AuthRequest, res) => {
  ...
  const pipeline = db.prepare(
    "SELECT status, COUNT(*) as count FROM perf_plans GROUP BY status"
  ).all() as any[];
  ...
  const overduePlans = db.prepare(`
    ...
    WHERE pp.deadline < ? AND pp.status NOT IN ('completed', 'draft')
  ...
  const bottlenecks = db.prepare(`
    ...
    WHERE pp.status IN ('pending_review', 'in_progress')
  ...
  const approverStats = db.prepare(`
    ...
    FROM perf_plans pp
    LEFT JOIN users u ON pp.approver_id = u.id
    WHERE pp.approver_id IS NOT NULL
  ...
  for (const approver of approverStats) {
    const logs = db.prepare(`
      ...
      JOIN perf_plans pp ON pl1.plan_id = pp.id AND pp.approver_id = ?
...
router.post('/nudge', authMiddleware, (req: AuthRequest, res) => {
  ...
  const plan = db.prepare('SELECT * FROM perf_plans WHERE id = ?').get(plan_id) as any;
