...
router.get('/:managerId/overview', authMiddleware, (req, res) => {
  ...
  const overview = members.map((m) => {
    const activePlans = db.prepare("SELECT COUNT(*) as count FROM perf_tasks WHERE (',' || assignee_id || ',' LIKE '%,' || ? || ',%') AND status NOT IN ('draft', 'completed')").get(m.id) as any;
    const completedPlans = db.prepare("SELECT COUNT(*) as count FROM perf_tasks WHERE (',' || assignee_id || ',' LIKE '%,' || ? || ',%') AND status = 'completed'").get(m.id) as any;
    const avgScore = db.prepare("SELECT AVG(score) as avg FROM perf_tasks WHERE (',' || assignee_id || ',' LIKE '%,' || ? || ',%') AND score IS NOT NULL").get(m.id) as any;
    const overduePlans = db.prepare("SELECT COUNT(*) as count FROM perf_tasks WHERE (',' || assignee_id || ',' LIKE '%,' || ? || ',%') AND deadline < date('now') AND status NOT IN ('completed')").get(m.id) as any;
